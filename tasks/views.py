    ...
    if request.method == 'GET':
        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                ...
                login(request, user)
                # ⬅️ Redirige a 'home' en lugar de 'sesiones'
                return redirect('home')
...
def crear_sesion(request):
    if request.method == 'POST':
        nombres = [n.strip() for n in request.POST.get(
            'participantes', '').split(',') if n.strip()]
        modo = request.POST.get('modo_propina')

        if not nombres or not modo:
            messages.error(
                request, "Debes ingresar participantes y un modo de propina.")
            return redirect('dashboard')

        sesion = Sesion.objects.create(estado='activa', modo_propina=modo, creador=usuario)


        for nombre in nombres:
            usuario, _ = Usuario.objects.get_or_create(nombre=nombre)
            Participante.objects.create(usuario=usuario, sesion=sesion)
...
def crear_sesion(request):
    if request.method == 'POST':
        nombres = [n.strip() for n in request.POST.get('participantes', '').split(',') if n.strip()]
        modo = request.POST.get('modo_propina')

        if not modo or not nombres:
            messages.error(request, "Debes ingresar participantes y elegir el modo de propina.")
            return redirect('dashboard')

        # ✅ Obtener o crear el Usuario (creador de sesión) SIN duplicar email
        usuario = Usuario.objects.filter(user=request.user).first()

        if not usuario:
            usuario_existente = Usuario.objects.filter(email=request.user.email).first()
            if usuario_existente:
                if not usuario_existente.user:
                    usuario_existente.user = request.user
                    usuario_existente.save()
                usuario = usuario_existente
            else:
        ...
        # Crear sesión con cantidad_personas y creador
        todos = [usuario.nombre] + [n for n in nombres if n != usuario.nombre]
        sesion = Sesion.objects.create(
            estado='activa',
            modo_propina=modo,
            cantidad_personas=len(set(todos)),
            creador=usuario
        )

        # Agregar al creador como participante
        Participante.objects.create(nombre=usuario.nombre, sesion=sesion)

        # Agregar otros participantes solo como texto
        for nombre in nombres:
            if nombre != usuario.nombre:
                Participante.objects.get_or_create(nombre=nombre, sesion=sesion)
...
def agregar_gasto(request, sesion_id):
    ...
    if request.method == 'POST':
        ...
        # Asegúrate que esta vista carga el template donde están los mensajes
        return redirect('tasks')
...
def eliminar_sesion(request, sesion_id):
    usuario = Usuario.objects.get(user=request.user)
    sesion = get_object_or_404(Sesion, id=sesion_id, creador=usuario)
    if request.method == 'POST':
        sesion.delete()
